...
import com.crcl.friend_ship.configuration.properties.ImageSizesProperties;
...
public class ImageProcessorImpl implements ImageProcessor {
    ...
    public void process(AuthenticatedQEvent<CreatePostPayload> event) {
        ...
        final var currentUser = userService.getCurrentUser().block();

        fileUploadResultStream
                .forEach(result -> storageClient.getObject(result.getName(), result.getEtag())
                        .subscribe(resource -> {
                            Consumer<ImageSize> action = process(result, resource, currentUser);
                            for (ImageSize imageSize : imageSizes) {
                                action.accept(imageSize);
                            }
                        }));
    }
