...
export class AuditService implements GenericCrud<Audit> {
  ...
  get(pageRequest: PageRequest): Observable<Page<Audit>> {
    return this.httpClient.get<Page<Audit>>(environment.api.audit.AUDIT_GET_PAGE, {params: {...pageRequest}});
  }

  getId(id: string | number): Observable<Page<Audit>> {
    return this.httpClient.get<Page<Audit>>(environment.api.audit.AUDIT_GET_BY_ID, {params: {id}});
  }
